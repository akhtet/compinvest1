...
def get_parameters():
	...
	days_hold = int(raw_input("Days to hold :"))
	amount = int(raw_input("Buy/sell amount :"))	

	return listname, dt_start, dt_end, lookback, n_target_a, n_target_b, fname, days_hold, amount	
...
def generate_orders(df_events, days_hold, amount):
	orders = []	
	n_events = 0

	for s_symbol in df_events.columns:
		for i in range(1, len(df_events.index)):
			#print df_events.index[i],

			if df_events[s_symbol].ix[i] == 1:
			
				date_hold = i + days_hold				
				if date_hold >= len(df_events.index):
					date_hold = -1

				orders.append([df_events.index[i], s_symbol, 'BUY', amount])
				orders.append([df_events.index[date_hold], s_symbol, 'SELL', amount])
...
def write_orders(orders, fname):
	...
	f = open(fname, 'w')
	for dt_day, s_symbol, s_action, n_amount in orders:
		f.write("%s,%s,%s,%d\n" % (dt_day.strftime("%Y,%m,%d"), s_symbol, s_action, n_amount)) 	
	f.close()	
...
if __name__ == '__main__':
	...
	s_listname, dt_start, dt_end, n_lookback, \
		n_target_a, n_target_b, s_fname, days_hold, amount = get_parameters()	
	...
	ep.eventprofiler(df_events, d_data, i_lookback=20, i_lookforward=20,
		s_filename=s_fname + '_events.pdf', b_market_neutral=True, b_errorbars=True, s_market_sym='SPY')

	orders = generate_orders(df_events, days_hold, amount)

	write_orders(orders, s_fname + '_orders.csv')
